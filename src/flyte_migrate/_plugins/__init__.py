...
def _transform_plugin_config_v1_to_v2(v1_config: Optional[Any]) -> Optional[Any]:
    ...
    cfg = None
    trans = [
        _transform_spark_config_v1_to_v2,
        _transform_ray_config_v1_to_v2,
        _transform_dask_config_v1_to_v2,
        _transform_pytorch_config_v1_to_v2,
    ]
    for tran in trans:
        cfg = tran(v1_config)
        if cfg is not None:
    ...
    raise NotImplementedError(
        ...
        f"Supported plugin types: Ray, Spark, Dask, PyTorch. Received config: {v1_config}"
    )
